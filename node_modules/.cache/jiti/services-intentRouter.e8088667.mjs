"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.IntentRouter = void 0;
...
class IntentRouter {
  ...
  skills)
  {
    ...
    if (isDesktopNote) {
      ...
      const tool = _toolRegistry.ToolRegistry.getTool('write_desktop_note');
    ...
    if (isTerminal) {
      ...
      if (dollarMatch) command = dollarMatch[1].trim();else
      if (execMatch) command = execMatch[1].trim();else
      command = text.trim();
      ...
      const tool = _toolRegistry.ToolRegistry.getTool('run_command');
    ...
    if (isWebSearch) {
      ...
      const targetSkill = skills.find((s) => s.id === targetSkillId);
      const isSkillActive = activeSkillIds.includes(targetSkillId);
      const tool = _toolRegistry.ToolRegistry.getTool('search_web');
    ...
    if (isFileWrite) {
      ...
      const targetSkill = skills.find((s) => s.id === targetSkillId);
      const isSkillActive = activeSkillIds.includes(targetSkillId);
      const tool = _toolRegistry.ToolRegistry.getTool('write_file');
    ...
    if (isMemorySave) {
      ...
      const targetSkill = skills.find((s) => s.id === targetSkillId);
      const isSkillActive = activeSkillIds.includes(targetSkillId);
      const tool = _toolRegistry.ToolRegistry.getTool('save_knowledge');
   ...
   */
  static generateBreakMessage(analysis) {
    return (
...
}exports.IntentRouter = IntentRouter; /* v9-12625bac69eacd36 */
